...
mod test {
    ...
    fn test_p_r_z() {
        ...
        let z = BIG::new_ints(&rom::CURVE_BNX);

        let mut t = p.clone();
        t.sub(&r);
        t.norm(); // David: this is required for printing to work.

        let z2 = BIG::modsqr(&z, &p);
        let z2_6 = BIG::modmul(&z2, &BIG::new_int(6), &p);

        assert_eq!(BIG::comp(&t, &z2_6), 0);
    }
