...
mod test {
    ...
    fn test_serde_compressed(pp: &ECP) {
        let mut bytes = [0; MODBYTES + 1];
        pp.tobytes(&mut bytes, true);
        let qq = ECP::frombytes(&bytes);
        assert!(pp.equals(&qq));
    }

    fn test_serde_uncompressed(pp: &ECP) {
    ...
    fn test_serde() {
        ...
        let pp = ECP::generator().mul(&x);
        test_serde_compressed(&pp);
        test_serde_uncompressed(&pp);
    }
}