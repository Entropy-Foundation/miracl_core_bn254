...
mod test {
    ...
    fn test_frob() {
        ...
        let mut res1 = g.mul(&p);
        res1.affine();

        let mut res2 = g.clone();
        let f = FP2::new_bigs(&BIG::new_ints(&rom::FRA), &BIG::new_ints(&rom::FRB));
        res2.frob(&f);
        assert!(res1.equals(&res2));

        let mut f2 = g.clone().x;
        f2.conj();
        f2.inverse(None);
        f2.mul(&res1.x);
        let mut f2_original = f.clone();
        f2_original.sqr();
        assert!(f2.equals(&f2_original));

        let mut f_new = g.clone().y;
        f_new.conj();
        f_new.mul(&f2);
        f_new.inverse(None);
        f_new.mul(&res1.y);
        assert!(f_new.equals(&f));
    }
}